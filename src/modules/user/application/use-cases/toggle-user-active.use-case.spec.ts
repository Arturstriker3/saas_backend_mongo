...
describe('ToggleUserActiveUseCase', () => {
  it('deactivates an active user account', async () => {
    ...
    const useCase = new ToggleUserActiveUseCase(repo);
    ...
    const result = await useCase.execute({ uuid: user.uuid });
  ...
  it('activates an inactive user account', async () => {
    ...
    expect(repo.updateActiveById.calls.length).toBe(1);
    expect(repo.updateActiveById.calls[0]?.[0]).toBe(user.uuid);
    expect(repo.updateActiveById.calls[0]?.[1]).toBe(true);
  ...
  it('throws when user does not exist', async () => {
    ...
    let error: unknown;
    try {
      await useCase.execute({ uuid: 'missing-user' });
    } catch (err) {
      error = err;
    }
  ...
  it('throws when trying to deactivate an admin account', async () => {
    const repo = createRepo();
    const useCase = new ToggleUserActiveUseCase(repo);
    const now = new Date('2025-01-01T10:00:00.000Z');
    const admin = {
    ...
    let error: unknown;
    try {
      await useCase.execute({ uuid: admin.uuid });
    } catch (err) {
      error = err;
    }
